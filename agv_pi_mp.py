...
class AGVCANRxAgent(object):
    ...
    def __init__(self, q_can_recv, q_can_recv_ctrl):
        ...
        self.ctrl_state = np.zeros(23, dtype=np.float64)
    ...
    def process_can_report_msg(self, msg):
        ...
        elif mid == 0x0B520101: # wheel 2 torque feedback
        ...
        elif mid == 0x0B520201: # wheel 2 speed feedback
        ...
        elif mid == 0x0B520101: # wheel 3 torque feedback
        ...
        elif mid == 0x0B520201: # wheel 3 speed feedback
        ...
        elif mid == 0x0B520101: # wheel 4 torque feedback
        ...
        elif mid == 0x0B520201: # wheel 4 speed feedback
        ...
        elif mid == 0x00000117: # open loop control drive by wire
        ...
        elif mid == 0x00000101: # open loop control drive by wire
        ...
        elif mid == 0x0E000201: # GPS latitude
        ...
        elif mid == 0x0E000202: # GPS longitude
        ...
        elif mid == 0x0E000203: # GPS velocity ENU
            
            gps_vel_enu = struct.unpack('<ii', msg.data)
            
            self.ctrl_state[8] = gps_vel_enu[0] # velocity component in east
            self.ctrl_state[9] = gps_vel_enu[1] # velocity component in north

        elif mid == 0x0E000204: # GPS position ENU
            
            gps_pos_enu = struct.unpack('<ii', msg.data)
            
            self.ctrl_state[10] = gps_pos_enu[0] # position component in east
            self.ctrl_state[11] = gps_pos_enu[1] # position component in north

        elif mid == 0x0E000205: # calculated speed by kinematic model
            
            vxy = struct.unpack('<hhhh', msg.data)
            
            self.ctrl_state[12] = vxy[0] # total velocity
            self.ctrl_state[13] = vxy[1] # vx
            self.ctrl_state[14] = vxy[2] # vy

        elif mid == 0x0E000206: # calculated angles by kinematic model and speed
            
            angles = struct.unpack('<hhhh', msg.data)
            
            self.ctrl_state[15] = angles[0] # heading angle
            self.ctrl_state[16] = angles[1] # yaw angle
            self.ctrl_state[17] = angles[2] # side slip

        elif mid == 0x0E000207: # calculated angles by kinematic model and speed
            
            errors = struct.unpack('<hhhh', msg.data)
            
            self.ctrl_state[18] = errors[0] # ey
            self.ctrl_state[19] = errors[1] # eh

        elif mid == 0x0E000208: # calculated angles by kinematic model and speed
            
            imu_state = struct.unpack('<hhhh', msg.data)
            
            self.ctrl_state[20] = imu_state[0] # r
            self.ctrl_state[21] = imu_state[1] # ay
    ...
    def process_can_report_msg_using_dbc(self, msg):
        ...
        if msg.arbitration_id == 0x0A000203:
            ...
            self.ctrl_state[9] = dmsg['GPS_VelN']

        if msg.arbitration_id == 0x0A000204:
            
            self.ctrl_state[10] = dmsg['GPS_VelE']
            self.ctrl_state[11] = dmsg['GPS_VelE']
    ...
    def run(self):
        ...
        self.can_dbc = cantools.database.load_file('./dbc/AGVPi8_Test.dbc')
...
class AGVCANTxAgent(object):
    ...
    def deliver_can_command_msg(self, command):
        ...
        gps_vn_msg_struct = self.can_dbc.get_message_by_name('GPS_VelN')
        ...
        self.can_bus.send(gps_vn_msg)

        gps_ve_msg_struct = self.can_dbc.get_message_by_name('GPS_VelE')
    ...
    def run(self):
        ...
        command_log_file_raw = open('./log/command_can.raw', 'w')
...
class AGVPi(object):
    ...
    def chassis_can_recv_thread(self):
        ...
        while not self.stop:
            ...
            if not self.q_can_recv.empty():
                ...
                self.ctrl_state = states_in_q[34:57]
    ...
    def construct_ctrl_command(self, command):
        ...
        # Control Enable, currently always 1
        command[2] = 1
        
        # Emergency Brake, currently always 0
        command[3] = 0
        
        command[4] = self.gps_pos_llh[1]
        command[5] = self.gps_pos_llh[2]
        
        command[6] = self.gps_vel_ned[1]
        command[7] = self.gps_vel_ned[2]
